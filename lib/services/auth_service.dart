...
class AuthService {
  ...
  Future<String?> signInWithGoogle() async {
    try {
      ...
      if (!userDoc.exists) {
        ...
        final encryptedName = EncryptionService.encryptText(displayName, userCredential.user!.uid);
  ...
  Future<void> saveOnboardingData({
    ...
    final uid = _firebaseAuth.currentUser!.uid;
    final encryptedName = EncryptionService.encryptText(name, uid);
    final encryptedAge = EncryptionService.encryptText(age, uid);
  ...
  Future<bool> updateUserData(Map<String, dynamic> data) async {
    try {
      ...
      if (data.containsKey('name')) {
        encryptedData['name'] = EncryptionService.encryptText(data['name'].toString(), uid);
      }
      if (data.containsKey('age')) {
        encryptedData['age'] = EncryptionService.encryptText(data['age'].toString(), uid);
      }
      ...
      if (e.code == 'not-found') {
        try {
          ...
          await _firebaseDb.collection('users').doc(uid).set(encryptedData, SetOptions(merge: true));
          return true;
        } catch (e2) {
